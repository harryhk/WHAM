...
hist_group::hist_group(ifstream & input, double tolerance, double temp, int histn, double histmin, double histmax, string calcF_M, int pmfn,  double pmfmin, double pmfmax ):tol(tolerance), num_bins(histn), hist_max(histmax), hist_min(histmin), pmf_num_bins(pmfn), pmf_max(pmfmax), pmf_min(pmfmin),  calcF_Method(calcF_M), dev_bias(NULL), dev_F(NULL), dev_ni(NULL), dev_F_old(NULL), n_bias_tot(0) {
	...
	for(int i=0; i!= num_windows; i++ ) {
		 ...
		 while( getline(input, line) ){
			...
			for(int j=0; j!=num_windows; j++){
                ...
                //traj_bias_n[i].data[traj_pos][j] = (float ) temp;
				
                /*if( i==39 && traj_pos==0 ) {
                    cout<<file_ns[i]<<" "<<traj_bias_n[i].data[traj_pos][j]<<endl;
                    //cout<<"haha"<<endl;
                    //cout<<temp<<endl;
                    //cout<<line<<endl;
                    if(j==39) exit(-1);
...
double hist_group::calc_bias( int win_b, int win_traj, int traj_p){
	...
	double coor= trajs[win_traj].data[traj_p];
	double dx= coor - loc;
